...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    let newCity = this._localStorageService.getCity();
    this.city = newCity[0].toUpperCase() + newCity.substring(1);
    this.getWeather();
  ...
  onSubmit(): void {
    if(this.deviceForm.valid) {
      ...
      this._deviceService.save(this.deviceForm.getRawValue()).subscribe({
        next: (resp) => {
          resp.city = resp.city[0].toUpperCase() + resp.city.substring(1);
          this.city = resp.city;
          this._localStorageService.setCity(resp.city);
    ...
    interval(1000).pipe(map( () => {
      this._arduinoService.getTemperature().subscribe({
        next: temp => {
          ...
          let currentTemp = temp !== 0 ? temp : this.lastTemp;
